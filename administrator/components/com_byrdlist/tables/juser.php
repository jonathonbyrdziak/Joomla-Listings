 ...
 * @since	1.0
 */
class TableJuser extends ResTable
{
	/**
	 * Unique id
	 *
	...
	*/
	...
	/**
	 * Get The Html Website Homepage
	 * 
	 */
	public function getWebsite()
	{
		//loading resources
		$records =& $this->getOneToMany('byrdlist_websites');
		
		if (!$records) return false;
		
		foreach ($records as $id => $record)
		{
			if ($record->homepage()) break;
		}
	...
	{
		//loading resources
		$community = $this->getCommunityRecord();
		...
		$html = '<img border="0" src="'.$community['thumb'].'" width="100" class="image-left" />';
	 ...
	 * 
	 */
	public function getHtmlAvatar( $width = 100 )
	{
		//loading resources
		$community = $this->getCommunityRecord();
		...
		//initializing variables
		$html = '<img border="0" src="'.$community['avatar'].'" width="'.$width.'" class="image-left" />';
	 ...
	 * 
	 */
	function getCommunityRecord()
	{
		//loading resources
    	$db =& JFactory::getDBO();
    	
    	$query = "SELECT * FROM `#__community_users` as `users`"
	   		." WHERE `users`.`userid` = '".$this->id()."'"
	   		." LIMIT 1;"
	   		;
